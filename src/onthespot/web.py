...
# Prefer websocket-only when gevent is available; fall back to polling on threading
_async_mode = "threading"
_transports = ["polling"]  # threading backend supports only polling
try:
    ...
    _async_mode = "gevent"
    _transports = ["websocket"]  # websocket-only to avoid long-poll polling spam
    logger.info("SocketIO using gevent async_mode (websocket-only transports)")
except Exception as e:
